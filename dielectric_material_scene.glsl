...
void main() {
    ...
    HitParams hit;
    const int bounces = 2; // Dont go beyond 20
    const int samples = 3;
    float energy = 0.5;
    float decay_multiplier = 0.9;
    float dielectric_clarity = 3.0;
    ...
    // Dielectric calculation with bounces
    vec3 total_color;
    for(int t=0; t<samples; t++){
        mul_color = diff_color;
        light = diff_light;
        vec3 random_bounce_color = mul_color * energy;
        prev_hit_point = hit_point;
        prev_normal = obj_normal;
        prev_ray_dir = r.direction - (prev_normal * (dot(r.direction, prev_normal) * 2.0));
        for(int i=0; i<bounces; i++){
            vec3 usphere_center = prev_hit_point + prev_ray_dir * (1.0 + dielectric_clarity);
            r.origin = prev_hit_point + prev_normal * 0.00001;
            r.direction = usphere_center + normalize(vec3(0.5-gold_noise(st,r.direction.x*10000.0),0.5-gold_noise(st,r.direction.y*10000.0),0.5-gold_noise(st,r.direction.z*10000.0)));
            r.direction -= prev_hit_point;
            r.direction = normalize(r.direction);
            hit = trace(r, pointlights, planes, spheres);
            energy = energy * decay_multiplier;
            mul_color *= hit.color;
            light += hit.light * energy;
            random_bounce_color += mul_color;
            prev_hit_point = hit.position;
            prev_normal = hit.normal;
            prev_ray_dir = r.direction;
        }
        random_bounce_color *= light;
        total_color += random_bounce_color;
    }

    mediump float samplesf = float(samples);
    total_color /= samplesf;
    // Dielectric ends
